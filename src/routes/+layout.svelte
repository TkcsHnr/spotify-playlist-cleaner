	...
	<div class="flex-none">
		{#if data.loggedIn}
			<div class="dropdown dropdown-end block">
				<button class="btn btn-circle overflow-clip size-12">
					<img src={data.userProfile?.images[0].url} alt="User profile" class="w-full h-full" />
				</button>
				<!-- svelte-ignore a11y_no_noninteractive_tabindex -->
				<div
					tabindex="0"
					class="dropdown-content mt-4 bg-base-200 rounded-box border border-base-content/10"
				>
					<ul class="menu min-w-36">
						...
						<li>
							<form action="/api/spotify/logout" method="post" class="p-0 w-full">
								<input type="submit" value="Log out" class="px-3 py-1.5 cursor-pointer text-error" />
							</form>
						</li>
					</ul>
				</div>
